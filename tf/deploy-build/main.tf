...
module "ssm_param_ecr_repo_url" {
  ...
  environment           = var.environment
  project               = var.project
  parameter_name        = "event_bus_table_ddl_lambda_ecr_repo_url"
  parameter_value       = module.ecr.ecr_repository_url
  parameter_description = "The ECR repository URL for the event-bus-table-ddl-lambda in the ${var.environment} environment."
}
